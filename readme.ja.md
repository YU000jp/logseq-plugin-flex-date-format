...
- ファイルに記録された日付フォーマットを、表示されるフォーマットから分離します。
...
1. 選択した日付フォーマットのスタイルに切り替える 🆕
   - ジャーナルリンクまたはジャーナルタイトル
   - "ローカライズ" モードまたは別の日付フォーマットを選択

1. ジャーナルリンクの曜日をローカライズ 🆙
   - "ローカライズ" モード以外
   - ユーザーの日付フォーマットに曜日が含まれていない場合、ジャーナルリンクにローカライズされた曜日を追加
   - フォーマットに含まれている場合、ジャーナルリンク内の曜日をローカライズ
...
選択した日付形式に切り替える
  ...
  - 上記のオプションが有効な場合、日付形式を選択: 選択
    ...
    - `yyyy_MM_dd`
    - `yyyy年MM月dd日`
  - ジャーナルリンクをホバーした際に相対的な時間を表示する: トグル
    - `true` デフォルト
    - `false`
